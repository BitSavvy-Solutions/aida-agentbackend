...
def _convert_delta_to_message_chunk(
    ...
    reason = cast(str, _dict.get("reason") or "")
    additional_kwargs: dict = {}
...
class BaseChatOpenAI(BaseChatModel):
    ...
    def _convert_chunk_to_generation_chunk(
        ...
        # Create response_metadata with generation ID and cost info
        response_metadata = {}
        if chunk_id := chunk.get("id"):
            response_metadata["generation_id"] = chunk_id

        # Add cost information to response_metadata if available
        if token_usage and "cost" in token_usage:
            response_metadata["cost"] = token_usage["cost"]
            if "cost_details" in token_usage:
                response_metadata["cost_details"] = token_usage["cost_details"]
        ...
        # The standard chunk conversion creates the initial message object.
        # Its `content` will be a string from the 'content' field in the delta.
        message_chunk = _convert_delta_to_message_chunk(
            choice["delta"], default_chunk_class
        )

        # --- START: MODIFICATION TO HANDLE CUSTOM 'reasoning' FIELD ---
        # Check for and handle the custom 'reasoning' field from the raw chunk delta.
        if delta := choice.get("delta"):
            if reasoning_text := delta.get("reasoning"):
                
                # Overwrite the message chunk's simple string content with the structured list.
                message_chunk.additional_kwargs["reasoning_content"] = reasoning_text
        # --- END: MODIFICATION ---

        generation_info = {**base_generation_info} if base_generation_info else {}

        # Add the generation ID to generation_info too
        if chunk_id := chunk.get("id"):
            generation_info["generation_id"] = chunk_id
        ...
        # Add response_metadata to the message chunk
        if response_metadata and isinstance(message_chunk, AIMessageChunk):
            # Merge with existing response_metadata if present
            if hasattr(message_chunk, "response_metadata") and message_chunk.response_metadata:
                message_chunk.response_metadata.update(response_metadata)
            else:
                message_chunk.response_metadata = response_metadata
    ...
    def _create_chat_result(
        ...
        llm_output = {
            "token_usage": f"lora{token_usage}",
